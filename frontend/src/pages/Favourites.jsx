...
import { Home, Briefcase, MapPin, Plus, Trash2, X, Save, Pencil } from "lucide-react";
...
function extractLatLng(fav) {
  const lat =
    fav?.lat ?? fav?.pickup_lat ?? fav?.latitude ?? fav?.location?.lat ?? fav?.location?.latitude ?? null;
  const lng =
    fav?.lng ?? fav?.pickup_lng ?? fav?.longitude ?? fav?.location?.lng ?? fav?.location?.longitude ?? null;
  return { lat, lng };
}
...
export default function Favourites() {
  ...
  const useAsPickup = (fav) => {
    ...
    const { lat, lng } = extractLatLng(fav);
    if (!address) return toast.error("This favourite has no address");

    navigate("/compare", { state: { pickup: { address, lat: lat ?? null, lng: lng ?? null } } });
  };
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <main className="mx-auto w-full max-w-5xl px-4 sm:px-6 lg:px-8 py-8">
        ...
        {favourites.length === 0 ? (
          <div className="text-center text-muted-foreground py-16">No favourites yet</div>
        ) : (
                  ...
                  <div className="flex justify-between items-start gap-4">
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-muted-foreground text-sm truncate">{addressText}</p>
                      </div>
                    </div>
